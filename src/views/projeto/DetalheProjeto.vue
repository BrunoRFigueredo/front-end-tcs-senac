    ...
    <form class="login">
      ...
      <div class="row">
        ...
        <div class="form-group col-md-6">
          ...
          <div>
            <select disabled v-model="projetoInsumo.status" name="status" id="status" class="form-select" aria-label="Default select example">
        ...
        <div class="modal-body">
          <div id="index-service" class="container">
            <table class="table text-center">
              ...
              <tbody>
                <tr v-for="projetoInsumo in dataInsumosProjeto" :key="projetoInsumo.id">
                  ...
                  <td>{{ projetoInsumo.status }}</td>
                  <span class="btn-whatsapp" @click="this.enviaMensagem(this.telefoneInstituicao, 'Olá! Gostaria de realizar doação do insumo ' + projetoInsumo.nomeInsumo + ' !')"><img src="../../assets/whatsapp1.png" height="20" width="20" alt=""></span>
                  <td v-if="this.idInstituicaoUsuario === this.instituicao.id">
                     <button @click="concluirInsumo(projetoInsumo.id)" class="btn">
                      <i class="bi bi-check-all"></i>
                    </button>
                    <button @click="deletarInsumo(projetoInsumo.id)" v-if="this.idInstituicaoUsuario === this.instituicao.id"
                      class="btn">
            ...
            <div class="row">
              ...
              <div class="col-md">
                <v-pagination v-model="paginaDesejadaInsumos" :pages="totalPaginaInsumos" :range-size="tamanhoPaginaInsumos"
            ...
            <table class="table text-center">
              ...
              <tbody>
                <tr v-for="projetoServico in data" :key="projetoServico.id">
                  ...
                  <td v-if="this.idInstituicaoUsuario === this.instituicao.id">
                    <button :disabled="!projetoServico.idVoluntario"
  ...
  methods: {
    ...
    async saveProjetoServico(projetoServico) {
      try {
        ...
        alert('Insumo cadastrado ao projeto!');
    ...
    async concluirInsumo(idInsumo){
      const {data} = await this.$crudProjetoInsumo.findById(idInsumo);
      this.projetoInsumo.id = data.id;
      this.projetoInsumo.quantidade = data.quantidade;
      this.projetoInsumo.status = 1
      this.projetoInsumo.projeto = data.projeto.id;
      this.projetoInsumo.insumo = data.insumo.id;

      try {
        this.$crudProjetoInsumo.save(this.projetoInsumo);
