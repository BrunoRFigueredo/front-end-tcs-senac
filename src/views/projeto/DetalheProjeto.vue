<template>
  <div id="detalhe-projeto" class="container">
    <div class="divDados">
      ...
      <div>
        <button class="btn btn-success" @click="showModal = true" style="margin-right: 70px"
                v-if="this.verificaLogado()">
          Cadastrar serviço
        </button>
        <button class="btn btn-success" @click="showModalInsumo = true" style="margin-right: 70px"
                v-if="this.verificaLogado()">
          Cadastrar Insumo
        </button>
        <button class="btn btn-success" @click="showModalPrestacao = true" style="margin-right: 70px"
                v-if="this.verificaLogado()">
      ...
      <div>
        <button type="button" class="btn btn-primary" @click="carregarProjetoServicos(this.id)" data-bs-toggle="modal"
                data-bs-target="#exampleModal" style="margin-right: 135px">
          Serviços
        </button>
        <button type="button" class="btn btn-primary" @click="carregarProjetoInsumos(this.id)" data-bs-toggle="modal"
                data-bs-target="#modalInsumos" style="margin-right: 135px">
          Insumos
        </button>
        <button type="button" class="btn btn-primary" @click="carregarPrestacaoContas(this.id)" data-bs-toggle="modal"
                data-bs-target="#modalPrestacao" style="margin-right: 135px">
          Prestação de contas
        </button>
      </div>
    </div>
  ...
  <Modal :model-value="showModal" title="Cadastro de serviço" id="modalServico" @update:modelValue="showModal = false">
    <form class="login">
      <div class="row">
  ...
  <Modal :model-value="showModalInsumo" title="Cadastro de insumos" id="cadastroInsumo"
         @update:modelValue="showModalInsumo = false">
    <form class="login">
      ...
      <div class="row">
        <div class="form-group col-md-12">
          ...
          <div>
            <InsumoSelect v-model="projetoInsumo.insumo"/>
          </div>
        </div>
      </div>
              ...
              <tr v-for="prestacaoConta in dataPrestacaoConta" :key="prestacaoConta.id">
                ...
                <td v-if="this.idInstituicaoUsuario === this.instituicao.id">
                  <button @click="deletarPrestacaoConta(prestacaoConta.id)"
                          v-if="this.idInstituicaoUsuario === this.instituicao.id"
                          class="btn">
  ...
  <Modal :model-value="showModalPrestacao" title="Cadastro de serviço" id="modalPrestacao"
         @update:modelValue="showModalPrestacao = false">
    <form class="login">
      <div class="row">
        <div class="form-group col-md-6">
          <div class="label">
          ...
          <div>
            <input @keyup="formatarData(prestacaoConta.dataPrestacao)" id="dataPrestacao" type="date"
                   class="form-control"
                   v-model="prestacaoConta.dataPrestacao" required>
          </div>
        </div>
        <div class="form-group col-md-6">
          <div class="label">
          ...
          <div>
            <input id="qtdAlimento" type="number" class="form-control"
                   v-model="prestacaoConta.qtdAlimento">
          </div>
        </div>
        <div class="form-group col-md-12">
          <div class="label">
          ...
          <div>
            <input id="vlArrecadado" type="number" class="form-control"
                   v-model="prestacaoConta.vlArrecadado">
          </div>
        </div>
      </div>
      <br>
          ...
          <div id="index-service" class="container">
            <table class="table text-center">
              ...
              <tr v-for="projetoInsumo in dataInsumosProjeto" :key="projetoInsumo.id">
                ...
                <td>
                  <span v-if="projetoInsumo.status === 'EM_ANDAMENTO'"
                        class="badge bg-primary">Em andamento</span>
                  <span v-if="projetoInsumo.status === 'CONCLUIDO'"
            ...
            <div class="row">
              ...
              <div class="col-md">
                <v-pagination v-model="paginaDesejadaInsumos" :pages="totalPaginaInsumos"
                              :range-size="tamanhoPaginaInsumos"
                              :hideFirstButton="true" :hideLastButton="true" active-color="#DCEDFF"
                              @update:modelValue="carregarProjetoInsumos"/>
              </div>
            </div>
          ...
          <div id="index-service" class="container">
            <table class="table text-center">
              ...
              <tr v-for="projetoServico in data" :key="projetoServico.id">
                ...
                <td>
                  <span v-if="projetoServico.statusServicoEnum === 'EM_ANDAMENTO'"
                        class="badge bg-primary">Em andamento</span>
                  <span v-if="projetoServico.statusServicoEnum === 'CONCLUIDO'"
                        class="badge bg-success">Concluído</span>
                  <span v-if="projetoServico.statusServicoEnum === 'PENDENTE'" class="badge bg-warning">Pendente</span>
                </td>
                <td v-if="this.idInstituicaoUsuario === this.instituicao.id">
                ...
                <td v-if="this.idInstituicaoUsuario === this.instituicao.id">
                  <button
                      :disabled="!projetoServico.idVoluntario || projetoServico.statusServicoEnum !== 'PENDENTE'"
...
export default {
  ...
  methods: {
    ...
    async carregarProjetoInsumos(idProjeto) {
      console.log('carregarProjetoInsumos');
      const dataInstituicao = await this.$crudInstituicao.findById(getLogado());
      this.telefoneInstituicao = dataInstituicao.data.telefone;
      api
    ...
    async saveProjetoServico(projetoServico) {
      try {
        ...
        alert('Serviço cadastrado ao projeto!');
    ...
    async deletarPrestacaoConta(idPrestacaoConta) {
      try {
        ...
        alert('Prestação deletado com sucesso');
      ...
      try {
        getClient()
            ...
            .then(response => {
              ...
              alert('Doação de insumo finalizada!');
            });
      } catch (erro) {
        alert(erro.response.data.message);
      }
